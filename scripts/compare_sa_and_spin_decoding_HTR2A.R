...
fs.overlay <- c(pet.5HT$`5HT2A`$lh, pet.5HT$`5HT2A`$rh)
...
for (i in 1:5) {
  ...
  filename <- paste0("HTR2A_spin", spin, ".png")
...
load(filename)
sa.5HT2A[fsavg6$medial.wall.verts$both] <- 0
...
for (i in 1:5) {
  sa <- sample(1:1000, 1)
  plot <- fs_plot(sa.5HT2A[sa, ], hemi = "lh")

  filename <- paste0("HTR2A_sa", sa, ".png")
  fs_save_plot_view_angle(
    plot,
    write.dir,
    filename,
...
gene.index <- which(abagen.genes$symbol == "HTR2A")
data.env <- new.env()
...
#   1:       HTR2A        3356 0.8798403 0.000999001 0.000999001 0.0009993206
...
# gene.symbol gene.entrez         r     p.perm p.maxT.adj p.fdr.adj
# <char>       <num>     <num>      <num>      <num>     <num>
#   1:       HTR2A        3356 0.8798403 0.05694306   0.986014         1
...
# gene.symbol gene.entrez         r      p.perm  p.maxT.adj  p.fdr.adj
# <char>       <num>     <num>       <num>       <num>      <num>
#   1:       HTR2A        3356 0.4809027 0.000999001 0.000999001 0.03809118
...
ggsave(
  filename = paste0(write.dir, "HTR2A_cor.pdf"),
  plot = p,
  height = 4.5,
  width = 6,
  units = "cm",
